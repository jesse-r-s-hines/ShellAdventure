...
class TestTutorial:
    def test_creation(self, tmp_path):
        tutorial = pytest.helpers.create_tutorial(tmp_path, {"puzzle1.py": SIMPLE_PUZZLES, "puzzle2.py": SIMPLE_PUZZLES}, f"""
            modules:
    ...
    def test_str_path_creation(self, tmp_path):
        # Create the files
        tutorial = pytest.helpers.create_tutorial(tmp_path, {"mypuzzles.py": SIMPLE_PUZZLES}, SIMPLE_TUTORIAL)
    ...
    def test_empty(self, tmp_path):
        with pytest.raises(Exception, match="Invalid config"):
            tutorial = pytest.helpers.create_tutorial(tmp_path, {"mypuzzles.py": SIMPLE_PUZZLES}, "")

    def test_missing_files(self, tmp_path):
        with pytest.raises(FileNotFoundError):
            tutorial = pytest.helpers.create_tutorial(tmp_path, {}, SIMPLE_TUTORIAL) # Don't make any puzzle files
        with pytest.raises(FileNotFoundError):
    ...
    def test_duplicate_module_names(self, tmp_path):
        with pytest.raises(Exception, match='Multiple puzzle modules with name "puzzle1.py" found'):
            tutorial = pytest.helpers.create_tutorial(tmp_path, {"puzzle1.py": SIMPLE_PUZZLES}, """
                modules:
                    - puzzle1.py
                    - puzzle1.py
                puzzles:
